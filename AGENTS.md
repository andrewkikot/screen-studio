...
- **Linux/Wayland**: on Wayland sessions (`WAYLAND_DISPLAY`/`XDG_SESSION_TYPE=wayland` + `DISPLAY` present) the main process RE-EXECS itself with a real `--ozone-platform=x11` argv flag (guarded by `SS_X11_REEXEC`, parent idles until child exits). Native-Wayland capture goes through xdg-desktop-portal ScreenCast, which pops GNOME's "select a monitor to share" dialog before every grab and makes globalShortcut unreliable — this was the "app dead until I pick a monitor" bug on Ubuntu. Do NOT use `app.commandLine.appendSwitch('ozone-platform', ...)` for this: Chromium picks the Ozone backend before main JS runs, so in-code switches are too late (Vesktop hit the same). Opt out with `SS_OZONE=wayland`. Do NOT add `--no-sandbox` via appendSwitch (see electron-builder bullet below).
