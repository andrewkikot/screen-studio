...
- **Linux/Wayland**: on Wayland sessions (`WAYLAND_DISPLAY` set) the app forces `--ozone-platform=x11` (XWayland) at startup. Native-Wayland capture goes through xdg-desktop-portal ScreenCast, which pops GNOME's "select a monitor to share" dialog before every grab and makes globalShortcut unreliable — this was the "app dead until I pick a monitor" bug on Ubuntu. Opt out with `SS_OZONE=wayland`. Do NOT add `--no-sandbox` via appendSwitch (see electron-builder bullet below).
